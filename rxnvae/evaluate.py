...
class Evaluator(nn.Module):
	...
	def decode_from_prior(self, rxn_latent, n, prob_decode=True):
		for i in range(n):
			product, reactions = self.model.rxn_decoder.decode(rxn_latent, prob_decode)
			if product != None:
	...
	def validate_and_save(self, train_rxn_trees, n=10000, output_file="generated_reactions.txt"):
		...
		with open(output_file, "w") as writer:
			for i in range(n):
				rxn_latent = torch.randn(1, self.latent_size)
				product, reactions = self.decode_from_prior(rxn_latent, 20)
				if product!= None:
					validity +=1
					print(i, validity/(i+1), "Product:",product, ", No Reaction:", len(reactions.split(" ")))
